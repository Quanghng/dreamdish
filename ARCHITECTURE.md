...
### 2. Génération Complète (Prompt + Image)
```
┌─────────────────────────────────────────────────────────────────────────────────────────────────────┐
│                              COMPLETE GENERATION FLOW                                                │
└─────────────────────────────────────────────────────────────────────────────────────────────────────┘

   User                    Client                    Server                 Mistral            Image Gen
    │                        │                         │                       │                   │
    ...
    │                        │                         │<────────┘             │                   │
    │                        │                         │                       │                   │
    │                        │                         │  11. Generate image (Google Gemini)       │
    │                        │                         │──────────────────────────────────────────>│
    │                        │                         │                       │                   │
    │                        │                         │  12. Image base64     │                   │
    │                        │                         │<──────────────────────────────────────────│
...
| Étape | Service | Modèle | Rôle |
|-------|---------|--------|------|
| 5-6 | Mistral AI | mistral-moderation-latest | Modération du contenu |
| 8-9 | Mistral AI | mistral-small-latest | Génération du prompt visuel |
| 11-12 | Google Gemini | gemini-2.5-flash-image | Génération d'image (principal) |
| 11-12 | Hugging Face | stable-diffusion-xl-base-1.0 | Génération d'image (fallback) |

### 4. Architecture Client-Serveur
```
┌─────────────────┐        ┌──────────────────┐        ┌─────────────┐        ┌──────────────┐
│  Client (React) │ -----> │ API Route (Next) │ -----> │  Mistral AI │        │ Google Gemini│
│  /components    │  HTTP  │  /app/api        │  SDK   │  (Prompts)  │        │  (Images)    │
│  /hooks         │        │  /lib            │        └─────────────┘        └──────────────┘
└─────────────────┘ <----- └──────────────────┘ -----------------------------> ┌──────────────┐
     JSON Request            Server-side only                                  │ Hugging Face │
     + Image URL                                                               │  (Fallback)  │
                                                                               └──────────────┘
```
